...
public class InnoKeyChainWrapper: NSObject {
    ...
    private func update(service: NSString, data: NSString, userAccount : NSString) {
        let dataFromString: NSData = data.data(using: String.Encoding.utf8.rawValue, allowLossyConversion: true)! as NSData
    ...
    public func getAllKeyChainItemsOfClass(_ serviceName: String) -> NSMutableArray {
        ...
        let query: NSMutableDictionary = NSMutableDictionary(objects: [kSecClassGenericPasswordValue, serviceName], forKeys: [kSecClassValue, kSecAttrServiceValue])
        ...
        for result in resultData! {
            var dict = ["password": self.loadPassword(accountname: result["acct"] as! NSString), "name": result["acct"]]
    ...
    public func renameAccount(_ newAccountName: String, _ oldAccountName: String) {
        ...
        let keychainQuery: NSMutableDictionary = NSMutableDictionary(objects: [kSecClassGenericPasswordValue, serviceKey, oldAccountName], forKeys: [kSecClassValue, kSecAttrServiceValue, kSecAttrAccountValue])
